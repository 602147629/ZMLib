package com.component
{
	...
	public class AutoSlideContainer extends Sprite
	{
		private var actualWidth:Number;
		private var actualHeight:Number;
		private var markRect:Bitmap;
		public function AutoSlideContainer(w:Number,h:Number)
		{
			super();
			actualWidth = w;
		...
		private var _offCount:int = 2;//偏移数据个数
...
//		}
		...
		private function move():void{
			if(!isStop){//不需要移动
				TimerManager.setIntervalOut(delay,moveRendener);
...
//				item.x -= _scrollSpeed;//开始移动
				TweenLite.to(item,.5,{x:item.x - (item.width + _hGap) * _columns,onComplete:checkHorizontalRendener,onCompleteParams:[item]});
			}
		}
		
		private function checkHorizontalRendener(item:BasicItemRenderer):void{
			var tCount:int = _columns + _offCount;
...
//				item.y -= _scrollSpeed;//开始移动
				TweenLite.to(item,.5,{y:item.y - (item.height + _vGap) * _rows,onComplete:checkVerticalRendener,onCompleteParams:[item]});
			}
		}
		
		private function checkVerticalRendener(item:BasicItemRenderer):void{
			var tCount:int = _rows + _offCount;
