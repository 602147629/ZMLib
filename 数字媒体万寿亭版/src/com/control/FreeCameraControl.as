package com.control
{
	...
	public class FreeCameraControl
	{
		...
		public function set keyBoardTurn(value:Boolean):void
		{
			_keyBoardTurn = value;
			if(value){
				if(_stage != null)_stage.addEventListener(Event.ENTER_FRAME,onLoop);
			}else{
				if(_stage != null)_stage.removeEventListener(Event.ENTER_FRAME,onLoop);
			}
		}
...
//			trace("关闭");
		...
		public function zoomCamera(isLarge:Boolean = false):void{
			if(isLarge){
				TweenLite.to(_freeCamera,.2,{distance:_freeCamera.distance - _perDistance});
...
//			trace("开启");
			cameraEnabled = true;
			timeId = setTimeout(checkMove,20);
