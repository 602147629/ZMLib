...
//		private var isTween:Boolean;
		private function moveTo(value:Vector3D,onComplete:Function = null,args:Array = null):void{
//			if(isTween)return;
			if(value.x == point3D.x && value.y == point3D.y && value.z == point3D.z){
				if(onComplete != null)args != null ? onComplete.apply(null,args) : onComplete();
				return;
			}
			TweenLite.to(this,.3,{overwrite:0,x:value.x,y:value.y,z:value.z,
...
//			point3D.x += value * Math.cos(horizontal) * Math.cos(vertical);
		 ...
		 * 恢复默认位置
		 */		
		public function reset(onComplete:Function = null,...args):void{
...
//			this.centerPoint3D = _centerPoint3D;
			TweenLite.to(this,.3,{distance:_defaultDistance});
			moveTo(_centerPoint3D,onComplete,args);
		}
