...
//			refresh();
		...
		private function convert():void
		{
			...
			if(hasIcon){
				PowerLoader.addEventListener("richImageComplete",loadComplete);
			}else{
				loadComplete();
			}
...
//			trace("scrollH:" + _textField.scrollH,"scrollV:" + _textField.scrollV);
			while ($_len--) 
			{
				$_info = $_arr[$_len];
				if($_info){
...
//							$_lineHeight = $_rect.height *0.5 > $_item.height? $_txtLineMetrics.ascent- $_item.height  : ($_rect.height - $_item.height)*0.5 + $_txtLineMetrics.descent * 0.5;// $_txtLineMetrics.ascent ;// + $_txtLineMetrics.descent * 0.5;
//							if($_lineHeight < 0){
//								return;
//							}
							$_item.visible = true;
							$_item.x =  $_rect.x + ($_rect.width - $_item.width)*0.5;
							$_item.y =  $_rect.y + ($_rect.height - $_item.height)*.5 ;//+ $_lineHeight;
						}else {
							$_item.visible = false;
						}
					}
		...
		private function setDelay():void
		{
			isDelay = false;
			refresh();
			if(this.parent == Vision.stage){
				Vision.stage.removeChild(this);
			}
			this.alpha = 1;
			loadComplete();
			trace("进行延迟50毫秒");
...
//		private function onDelay(e:Event):void{
//			refresh();
//		}
		//-------------------------------------------------------------------------
		// TextFormat
...
//			_defaultFormat.font = _fontName;
		...
		public function set leading(value:Number):void{
			...
			df.leading = _leading;
			_textField.defaultTextFormat = df;
		}
		private function setFormat($id:int):void
		{
			...
			$_item = $_info.item;
			$_format = new TextFormat(_fontName);
			$_format.kerning = true;
			$_format.leading = _leading;
			var size:int = sizeFunc($_item.height);
			$_format.size = size;
			$_format.target = $_info.iconStr + SEPARATOR + $_info.iconType + SEPARATOR + $_item.name;
//			$_format.letterSpacing = $_item.width - getTxtWidth($_item.height);
			...
			var sourceHeight:Number = _textField.height;        //保存原高度，用于后面还原回来
			_textField.height = _textField.textHeight;
			var $_rect:Rectangle = _textField.getCharBoundaries($_info.index);
			if($_rect != null){
				$_format.letterSpacing = $_item.width - $_rect.width;
			}else{
				$_format.letterSpacing = $_item.width - getTxtWidth($_item.height);
				trace("$_rect为null");
			}
			_textField.height = sourceHeight;
			_textField.setTextFormat($_format, $_info.index);
		...
		private function getTxtWidth($size:int):int
		{
			...
			var $_format:TextFormat = new TextFormat(/*_fontName*/);
			$_format.leading = _leading;
//			$_format.kerning = true;
			$_txt.text = PLACEHOLDER;
			$_format.size = sizeFunc($size);
			$_txt.setTextFormat($_format);
			return $_txt.textWidth-2;
		}
		public var sizeFunc:Function = defaultSize;
		private function defaultSize(size:int):int{
			return size;//(size - 5) / 11 * 10;
		}
...
//				PowerLoader.loadFile($iconInfo.iconUrl,onImageLoaded,$iconInfo);
				PowerLoader.loadFileWithEvent($iconInfo.iconUrl,onImageLoaded,"richImageComplete",$iconInfo);
//				PowerLoader.addCompleteEvent(loadComplete);
		...
		{
			if(!isDelay)dispatchEvent(new Event(Event.COMPLETE));
			//不延迟直接抛出
//			setTimeout(dispatchEvent,2000,new Event(Event.COMPLETE));
		}
		
		private function onImageLoaded(bmp:Bitmap,$iconInfo:Object):void{
			var $item:Bitmap = new Bitmap(bmp.bitmapData);
			...
			//trace($item.width);
			var $_id:int = _spriteMap.length-1;
			
			_spriteContainer.addChild(sp);
			setFormat($_id);
			
			if(!isDelay){
				setTimeout(setDelay,50);
				isDelay = true;
				...
				if($_info){
					...
					if($_item){
						...
						if ( $_info.index < 0 || $_info.index >= _textField.length) {
							...
							if ($_info.status == STATUS_NORMAL && $_textFormat.target != $_info.textFormat.target) {	
								_spriteContainer.removeChild($_item);
								$_arr[$_len] = null;
								$_info = null;
								//trace("remove2");
...
//		}
		...
		private function drawRectGraphics($container:Object, $w:int = 10, $h:int = 10, $isClear:Boolean = false,  $alpha:int = 1):void 
		{
