package com.utils
{
	...
	public class StarlingLoader
	{
		 ...
		 * @param args
		 */		
		public static function loadImageFile(url:String,isTexture:Boolean = true,loadFunc:Function = null,...args):void{
			if(url == null || StringUtil.trim(url) == ""){
				trace("加载地址为null");
				return;
			}
			initFile(url,isTexture,loadFunc,args);
		}
//		/**
//		 * 以二进制数据方式加载目标
//		 * @param url
...
//		}
		...
		private static function initFile(url:String,isTexture:Boolean,loadFunc:Function,args:Array):void
		{
			var data:Object = getFile(url);
			if(data != null){
				loadComplete(data,loadFunc,args);
			}else{
				checkFile(url,isTexture,loadFunc,args);
			}
		}
		...
		private static function checkFile(url:String, isTexture:Boolean, loadFunc:Function, args:Array):void
		{
...
//			if(loadDic == null)loadDic = new Dictionary(true);
			...
			if(tempUrl.lastIndexOf('.png') > 0 || tempUrl.lastIndexOf('.jpg') > 0 || 
				...
				CacheUtils.loadByteAndSave(url,cutAddress,loadImageByte,url,isTexture,loadFunc,args);
//				loadImage(url,loadFunc,args);
 			}
			loadProgress();
		}
		
		private static function loadImageByte(byte:ByteArray,url:String,isTexture:Boolean,
											  loadFunc:Function,args:Array):void
		{
			...
			pushLoader(loader,url,isTexture,loadFunc,args,byte);
		}
		/**
		 * 注册帧循环 每帧监听加载进度
		 */		
...
//			setTimeout(endDispatch,3000);
		 ...
		 * @param args
		 */		
		private static function loadImage(url:String, isTexture:Boolean, loadFunc:Function, args:Array):void
		{
			if(urlDic[url] !== undefined){
				loader = urlDic[url];
			}else{
...
//				loader.contentLoaderInfo.addEventListener(ProgressEvent.PROGRESS,onAssetsProgress);
			...
			pushLoader(loader,url,isTexture,loadFunc,args);
		}
...
//					if(bd)
					...
					if(loadVo.isTexture){//是否转换成材质
						var te:Texture = Texture.fromBitmap(asset as Bitmap);
						var bd:BitmapData = (asset as Bitmap).bitmapData;
//						bd.dispose();//销毁像素
						(asset as Bitmap).bitmapData = null;
						asset = te;//新的材质
					}
				}
		 ...
		 * @param loader
		 */		
		private static function pushLoader(loader:Object,url:Object, isTexture:Boolean, 
										   loadFunc:Function, args:Array,byte:ByteArray = null):void
		{
			...
			loadVector.push(new LoadVo(url,isTexture,loadFunc,args,byte));
			urlDic[url] = loader;
		}
...
class LoadVo
{
	public function LoadVo(url:Object, isTexture:Boolean ,loadFunc:Function, args:Array,byte:ByteArray = null):void{
		this.url = url;
