package com.utils
{
	...
	public class GraphicsUtil
	{
		...
		public static function drawRegularStar(g:Graphics,
				sides:int,minR:Number,maxR:Number,color:uint = 0,isLine:Boolean = false,thickness:int = 2):void{
			if(sides <= 2){
...
//			}
			...
			for (var i:int = 0; i < sides; i++) 
			{
				...
				if(i == 0){
					g.moveTo(x1,y1);
					if(isLine)fPoint = new Point(x1,y1);
				}else{
					g.lineTo(x1,y1);
				}
				//--------- 内外交替连线 ----------
...
//				g.beginFill(color);//开始填充
				...
				for (i = 0; i < sides; i++) 
				{
					angle = 360 / sides * i;
					//计算每个顶点分到的角度
					x1 = maxR * Math.sin(angle / 180 * Math.PI);
					y1 = -maxR * Math.cos(angle / 180 * Math.PI);
					//通过圆周公式换算成坐标 外顶点坐标(x1,y1)
					if(i == 0){
						g.moveTo(x1,y1);
					}else{
