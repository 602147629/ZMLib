...
package feathers.controls
{
	...
	public class ScreenNavigator extends FeathersControl
	{
		...
		public function get normalHeight():Number
		{
		 ...
		 * Constructor.
		 */
		public function ScreenNavigator(w:Number = 0,h:Number = 0)
		{
			super();
			if(!SIGNAL_TYPE)
			{
		 ...
		 * @private
		 */
		protected var _screens:Vector.<ScreenVo> = new Vector.<ScreenVo>();
		 ...
		 */
		public function showScreen(id:String,noTransition:Boolean = false):DisplayObject
		{
			const item:ScreenNavigatorItem = getScreen(id);
			if(!item)
			{
			...
			if(this._transitionIsActive)
			{
				if(this._previousScreenInTransition)
				{
					...
					{
						const s:IScreen = IScreen(this._previousScreenInTransition);
						s.screenID = null;
						s.owner = null;
					}
...
//				return null;
			...
			if(noTransition){//不需要过度
				var trans:Function = defaultTransition;
			}else{
				trans = this.transition;
			}
			this.dispatchEventWith(FeathersEventType.TRANSITION_START);
			trans(this._previousScreenInTransition, this._activeScreen, transitionComplete);
		...
		protected function clearScreenInternal(displayTransition:Boolean):void
		{
			...
			const item:ScreenNavigatorItem = getScreen(this._activeScreenID);
			const events:Object = item.events;
		...
		public function removeScreen(id:String):void
		{
			...
			for (var j:int = _screens.length - 1; j >= 0; j--) 
			{
				if(_screens[j].id == id){
					_screens.splice(j,1);
					break;//切除掉
		...
		{
			for each (var svo:ScreenVo in _screens) 
			{
				if(svo.id == id)return svo.screenItem;
			}
			return null;
		}
		...
		public function getScreenIDs(result:Vector.<String> = null):Vector.<String>
		{
			...
			for each(var svo:ScreenVo in this._screens)
			{
				result.push(svo.id);
			}
			return result;
		...
		protected function transitionComplete():void
		{
			...
			if(this._previousScreenInTransition)
			{
				const item:ScreenNavigatorItem = getScreen(this._previousScreenInTransitionID);
...
//				this.showScreen(this._nextScreenID);
		...
		public function getScreenIndex(dpo:DisplayObject):int{
			for (var i:int = _screens.length - 1; i >= 0; i--) 
			{
				if(_screens[i].screenItem.getScreen() == dpo){
					return i;
				}
			}
			return 0;
...
class ScreenVo{
	...
	public var screenItem:ScreenNavigatorItem;
	public function ScreenVo(id:String,screenItem:ScreenNavigatorItem){
		this.id = id;
