package away3d.loaders.parsers
{
	...
	public class OBJParser extends ParserBase
	{
		...
		private var objPath:String;//记录此Obj的路径
		/**
		 * @inheritDoc
		 */
		...
		private function parseLine(trunk:Array):void
		{
			switch (trunk[0]) {
				case "mtllib":
					_mtlLib = true;
					_mtlLibLoaded = false;
					var nameList:Array = _fileName.split("/");
//					_fileName = nameList.pop();
					nameList.pop();
					objPath = nameList.join("/");
					loadMtl (objPath + '/' + trunk[1]);
					break;
				case "g":
					createGroup(trunk);
					break;
		...
		private function parseMtl(data:String):void
		{
			...
			for(var i:uint = 0;i<materialDefinitions.length;++i){
				...
				if(mapkd != ""){
					...
					addDependency(_lastMtlID, new URLRequest(objPath + '/' + mapkd));
