...
//					_loader.loadBytes(_byteData);
					...
					PowerLoader.convertByte(_byteData,onComplete);
//					_doneParsing = true;
				}
			}
			return _doneParsing;
		}
		private static var textureDic:Dictionary = new Dictionary(true);
		private function onComplete(bmp:Bitmap):void
		{
			var bmd:BitmapData = bmp.bitmapData;
			var asset:BitmapTexture;
			if (!TextureUtils.isBitmapDataValid(bmd)) {
				var bmdAsset:BitmapDataAsset = new BitmapDataAsset(bmd);
				bmdAsset.name = _fileName;
				dispatchEvent(new AssetEvent(AssetEvent.TEXTURE_SIZE_ERROR, bmdAsset));
				bmd = new BitmapData(8, 8, false, 0x0);
			...
			if(textureDic[bmd] !== undefined){
				asset = textureDic[bmd];
			}else{
				asset = new BitmapTexture(bmd);
				textureDic[bmd] = asset;
//				TextureMaterial(asset).alphaBlending = true
				trace('转换结束的文件名:' + _fileName);
			}
			finalizeAsset(asset, _fileName);
			_doneParsing = true;
		}
