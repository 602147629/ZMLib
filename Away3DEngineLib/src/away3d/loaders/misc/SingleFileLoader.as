...
//		private var pvo:ParserVo;
		...
		public function load(urlRequest : URLRequest, parser : ParserBase = null, loadAsRawData : Boolean = false) : void
		{
			...
			PowerLoader.loadUrl(urlRequest,dataFormat,loadComplete);
		 ...
		 */
		public function parseData(data : *, parser : ParserBase = null, req : URLRequest = null,
								  ns:String = null) : void
		{
			_nameSpace = ns;
			if (data is Class)
		...
		private static var parseDic:Dictionary = new Dictionary(true);
		/**
...
//			if(parseDic[data] !== undefined){
//				pvo = parseDic[data];
//				_parser = pvo.parser;
//				trace('已经转换过了:' + _req.url + '///' + _parser);
//				if(pvo.hasParser){
//					onParseComplete(null);//已经完成了
//				}else{
//					pvo.addParserFunc(onParseComplete);
//				}
//				return;
//			}
			if (!_parser){
			...
			if(_parser){
				...
				else _parser._fileName = _nameSpace;
//				trace('开始转换:' + (_req != null ? _req.url : ''));
...
//				parseDic[data] = pvo;
		...
		{
			//			_parser.removeEventListener(event.type,onReadyForDependencies);
			dispatchEvent(event.clone());
			//			if(_parser.hasEventListener(event.type))_parser.dispatchEvent(event);
		}
		
		private function onAssetComplete(event : AssetEvent) : void
		{
			//			_parser.removeEventListener(event.type,onAssetComplete);
		...
		{
			//			_parser.removeEventListener(event.type,onTextureSizeError);
			this.dispatchEvent(event.clone());
			//			if(_parser.hasEventListener(event.type))_parser.dispatchEvent(event);
		}
		 ...
		 */
		private function onParseComplete(event : ParserEvent = null) : void
		{
//			if(event != null){
//				pvo.hasParser = true;//已经加载完毕了
//				dispose(pvo);
//			}
			...
			//			_parser.removeEventListener(event.type,onParseComplete);
			//			if(_parser.hasEventListener(event.type))
			//				_parser.dispatchEvent(event);
		}
		
//		private function dispose(pvo:ParserVo):void
//		{
//			for each (var func:Function in pvo.parserList) 
//			{
//				func();
//			}
//			if(pvo.parserList != null)
//			pvo.parserList.length = 0;
//		}
	}
}
...
//	public var hasParser:Boolean;//是否已经转换完毕
//	public var parserList:Vector.<Function>;//和此加载器有关的数据
//	
//	public function addParserFunc(func:Function):void{
//		if(parserList == null){
//			parserList = new Vector.<Function>();
//		}
