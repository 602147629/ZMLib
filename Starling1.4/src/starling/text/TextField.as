...
package starling.text
{
    ...
    public class TextField extends DisplayObjectContainer
    {
		...
		protected var mLeading:Number;//行间距
		...
		private static var _registerName:String = "";
		
		public static function get registerName():String
		{
			return _registerName;
		}
		public static function registerFont(fName:String):void{
			_registerName = fName;
		}
		
		private static var _registerBoldName:String = "";
		public static function registerBoldFont(fName:String):void{
			_registerBoldName = fName;
		}
                                  ...
                                  fontSize:Number=12, color:uint=0x0, bold:Boolean=false)
        {
            ...
            mKerning = true;
            mBold = bold;
            mAutoSize = starling.text.TextFieldAutoSize.NONE;
		...
		private var _turncateToFit:Boolean;//自动切断末尾的字符 用"..."代替
		public function get turncateToFit():Boolean
		{
			return _turncateToFit;
		}
		public function set turncateToFit(value:Boolean):void
		{
			_turncateToFit = value;
		}
		
		public static function getTextWidth(text:String,fontSize:Number):Number{
			var textFormat:TextFormat = sNativeTextField.defaultTextFormat;
        ...
        protected function createRenderedContents():void
        {
            ...
            if (mImage == null) 
            {
				...
				mImage.smoothing = TextureSmoothing.BILINEAR;
		...
		private function checkCate():void
		{
			sNativeTextField.autoSize = flash.text.TextFieldAutoSize.LEFT;
			sNativeTextField.multiline = false;
			var scale:Number  = Starling.contentScaleFactor;
			...
			var maxInter:int = width / mFontSize;//最大寻找间隔因子
			var startIndex:int = 0;//起始位置
			sNativeTextField.text = "";
			while(startIndex < mText.length){
				var preText:String = sNativeTextField.text;//添加前的文字
				sNativeTextField.appendText(mText.slice(startIndex,startIndex + maxInter));
				if(sNativeTextField.height > height){//超过了说明在这个间隔内
					sNativeTextField.text = preText;//内容重置
					startIndex = preIndex;//回到原先位置
					maxInter /= 2;//减少一半间隔
					if(maxInter <= 3){
						sNativeTextField.appendText("...");//最后添加省略号
						break;
					}
				}else{
					var preIndex:int = startIndex;//原先的位置
				}
				startIndex += maxInter;
			}
			//			return sNativeTextField.text;
			//			mText//现有的文本
		}
        ...
        private function renderText(scale:Number, resultTextBounds:Rectangle):BitmapData
        {
			...
			if(_autoSizeHeight){
				...
				sNativeTextField.autoSize = flash.text.TextFieldAutoSize.NONE;
				sNativeTextField.height = height;
				sNativeTextField.multiline = true;
			}
			if(!_turncateToFit)
				sNativeTextField.text = mText;//显示正常文本
			//            sNativeTextField.filters = mNativeFilters;
			if(_autoSizeHeight){
				//				this.width = width = sNativeTextField.textWidth;
				this.height = height = sNativeTextField.height;
			}
        ...
        private function createComposedContents():void
        {
            ...
            if (mAutoSize != starling.text.TextFieldAutoSize.NONE)
            {
        ...
        {
            return mAutoSize == starling.text.TextFieldAutoSize.HORIZONTAL || 
                   mAutoSize == starling.text.TextFieldAutoSize.BOTH_DIRECTIONS;
        }
        
        private function get isVerticalAutoSize():Boolean
        {
            return mAutoSize == starling.text.TextFieldAutoSize.VERTICAL || 
                   mAutoSize == starling.text.TextFieldAutoSize.BOTH_DIRECTIONS;
        }
